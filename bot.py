...
CARGO_AUTORIZADO_SOLICITAR = 1469536548579049622
...
class ModalConvite(discord.ui.Modal):
    ...
    embed=embed,
    view=BotaoRegularizarSSP(self.membro.id)
)
...
class PainelAvaliacao(discord.ui.View):
    ...
    @discord.ui.button(label="Enviar Convite", style=discord.ButtonStyle.success)
    async def aprovar(self, interaction: discord.Interaction, button: discord.ui.Button):

        if interaction.message.id in pedidos_resolvidos:
            await interaction.response.send_message("Pedido já finalizado.", ephemeral=True)
            return
    ...
    @discord.ui.button(label="Recusar Convite", style=discord.ButtonStyle.danger)
...
class BotaoRegularizarSSP(discord.ui.View):
    def __init__(self, membro_id):
        super().__init__(timeout=None)
        self.membro_id = membro_id

    @discord.ui.button(label="Regularizar SSP", style=discord.ButtonStyle.gray)
    async def regularizar(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if cargo_sem_ssp not in membro.roles:
            await interaction.response.send_message(
                "Esse membro já está regularizado.",
                ephemeral=True
            )
            return
